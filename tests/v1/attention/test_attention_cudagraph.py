...
def run_attention_backend(backend: _Backend, kv_cache_spec: FullAttentionSpec,
                          ...
                          use_cudagraph: bool = True) -> torch.Tensor:
...
BATCHES = (
    ...
    ([32, 40], [8, 8]),
    ([32, 40, 48, 56], [1, 1, 5, 5]),
    ([128, 256, 512, 1024, 128, 256, 512, 1024], [1, 1, 1, 1, 1, 1, 1, 1]),
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="Runtime (ms)",
        plot_name="Benchmark Attention Backends with CUDA Graph",
        args={},
    )
)
def benchmark(backend, model, seq_lens, query_lens):
    ...
    if backend == _Backend.FLEX_ATTENTION:
    ...
    all_close = torch.allclose(backend_output,
                               ...
                               atol=atol)
    if not all_close:
        print(f"[{backend}] output differs from SDPA baseline. "
              f"Max diff: {max_diff:.6f} (rel: {max_rel_diff:.6f})")
        print(f"[{backend}] output: {backend_output}")
        print(f"[{backend}] SDPA baseline: {sdpa_output}")
